  ...
  return (
    <section className="bg-col">
      <div className="container max-w-[1100px] px-4">
        <div className="flex flex-col items-center justify-center pt-2 sm:pt-12 md:pt-16 pb-8 md:pb-10">
          <motion.h2 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
            {personas.map((persona, index) => (
              ...
              >
                <MagicCard
                className='bg-[#30303066] bg-opacity-50 backdrop-blur-xl text-left px-6 sm:px-7 md:px-8 pt-6 sm:pt-7 md:pt-8 pb-8 sm:pb-9 md:pb-10 rounded-[20px] sm:rounded-[25px] md:rounded-[30px] border-0 cursor-pointer hover:bg-[#30303099] transition-all duration-300'
                  gradientColor={theme === "dark" ? "#7d22ff" : "#7d22ff4d"}
                >
                  <img src={persona.imgSrc} className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 mb-5 sm:mb-6 md:mb-7" />
                  <h3 className="f-Nohemi-Light-BF6438cc583f70b text-[#FCFCD8] text-[22px] sm:text-[24px] md:text-[26px] lg:text-[28px] leading-tight font-normal mb-3 sm:mb-4">{persona.title}</h3>
                  <p className="f-Nohemi-Light-BF6438cc583f70b text-[#FCFCD880] text-[14px] sm:text-[15px] md:text-[16px] leading-relaxed font-normal">{persona.description}</p>
                </MagicCard>
              </motion.div>
            ))}
          </div>
